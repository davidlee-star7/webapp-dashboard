...
(function (jQuery) {
                ...
                for (var j = 0; j < data[i].series.length; j++) {
                    seriesDiv
                        .append(jQuery("<div>", { "class": "ganttview-vtheader-series-row" })
                            .append(
                            function() {
                                switch(data[i].series[j].priority){
                                    case 'high': var priority_color = 'md-bg-red-800'; break;
                                    case 'medium': var priority_color = 'md-bg-orange-800'; break;
                                    case 'low': var priority_color = 'md-bg-green-800'; break;
                                    default: var priority_color = 'md-bg-green-800'; break;
                                }

                                var $tType = '';
                                if($ttype = data[i].series[j].task_type){
                                    switch($ttype){
                                        case 'Randomise' :  $tType = "<span class='uk-badge uk-badge-danger pull-right'>Randomise</span>"; break;
                                        case 'Workflow' :   $tType = "<span class='uk-badge uk-badge-success pull-right'>Workflow</span>"; break;
                                    }
                                }

                                var ico =
                                    (data[i].series[j].contact_type)
                                        ?   "<div class='md-user-letters "+priority_color+" gantt-task-icon'><i class='white material-icons'>"+data[i].series[j].contact_type+"</i></div>"
                                        :   "";
                                ...
                                return ico + $tType +
                                    ...
                                    moment(data[i].series[j].start,"DD/MM/YYYY").format('D MMM') +
                                    " - " +
                                    moment(data[i].series[j].end,"DD/MM/YYYY").format('D MMM') +
                                    "</span>" +
                                    "</div>"
                            }
                        )
        ...
        function addBlocks(div, data, cellWidth, start) {
            ...
            for (var i = 0; i < data.length; i++) {
                for (var j = 0; j < data[i].series.length; j++) {
                    ...
                    var offset = DateUtils.daysBetween(moment(start).format('DD/MM/YYYY'), series.start);
    ...
    var Behavior = function (div, opts) {
        ...
        function bindBlockResize(div, cellWidth, startDate, callback) {
            $blocks = div.find("div.ganttview-block");
            $.each($blocks, function(){
                $options = ($(this).data("block-data"));
    ...
    var DateUtils = {
        ...
        daysBetween: function (start, end) {
            ...
            if (moment(start, "DD/MM/YYYY")._d.getYear() == 1901 || moment(end, "DD/MM/YYYY")._d.getYear() == 8099) { return 0; }
        ...
        getBoundaryDatesFromData: function (data, minDays) {
            var minStart = maxEnd = new Date();
            for (var i = 0; i < data.length; i++) {
                for (var j = 0; j < data[i].series.length; j++) {
                    var start = moment(data[i].series[j].start, "DD/MM/YYYY");
                    var end = moment(data[i].series[j].end, "DD/MM/YYYY");
