...
var buttonToggleClick = function(parent){
...
var buttonToggleAction = function(button)
{
    var activeBtn = button.find('.btn.active');
    var activeToggle = activeBtn.data('toggle');
    activeBtn.find('input[type="radio"]').not(':checked').prop("checked", true);
    activeBtn.find('input[type="checkbox"]').not(':checked').prop("checked", true);
    var togSplit = activeToggle.split(':');
    if (togSplit[0].indexOf("show") >= 0){
        if(togSplit[0].indexOf("class") >= 0){
            $(document).find('.'+togSplit[1]).show();
        }
        else if(togSplit[0].indexOf("id") >= 0){
            $(document).find('#'+togSplit[1]).show();
        }
    }
    else if (togSplit[0].indexOf("hide") >= 0){
        if(togSplit[0].indexOf("class") >= 0){
            $(document).find('.'+togSplit[1]).hide();
        }
        else if(togSplit[0].indexOf("id") >= 0){
            $(document).find('#'+togSplit[1]).hide();
        }
...
+function ($) { "use strict";
  ...
  $(document).on('click.app.bjax.data-api', '[data-bjax], .nav-primaryx a', function (e) {
...
+function ($) {
  ...
  $(function(){
      $.each($('.btn-toggle'),function(){
          buttonToggleAction($(this));
      });

      $(document).on('click', '.btn-toggle .btn',function(e) {
          $parent = $(this).parent('.btn-toggle');
          e.preventDefault();
          buttonToggleClick($parent);
          buttonToggleAction($parent);
      });
    ...
    // ajax modal
    $(document).on('click', '[data-toggle="ajaxModal"], .ajaxModal',
      function(e) {
        $('#ajaxModal, .modal, .modal-backdrop, .bootstrap-datetimepicker-widget').remove();
        deleteButton = $(this).attr('href')=='/confirm-delete'?true:false;
        e.preventDefault();
        var $this = $(this)
        ...
        $('body').append($modal);
        $modal.modal();
        $modal.load($remote, function(data){
            ...
            if(deleteButton){
                $(this).find('a.button-delete').attr('href',($this.data('action') || $this.data('href')));
            }
        });
      }
    );
