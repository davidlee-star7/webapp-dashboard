...
class ComplianceDiary extends  Modules
{
    public $unitStaffs;
    protected $options = [];
    ...
    {
        $this->layout = \View::make($this->layout);
        $submitted = \Model\ComplianceDiarySubmitted::find($id);
    ...
    public function getDatatable()
    {
        ...
        $submitted = \Model\ComplianceDiarySubmitted::whereUnitId($unitId)->orderBy('id','desc')->get();
            ...
            foreach ($submitted as $record)
            {
                ...
                $options[] = [
                    ...
                    \HTML::ownOuterBuilder(
                        \HTML::mdActionButton($record->id,'compliance-diary','submitted','search', 'View Details')
                    ),
                ];
            }
    ...
    public function postCreate()
    {
        ...
        if(!(count($errCustom))) {
            $new   = new \Model\ComplianceDiaryTasks();
            $new -> fill(\Input::all());
            $save = $new -> save();
            if($save){
                \Services\ComplianceDiary::createTasksItems($new);
    ...
    {
        $checklist = \Services\CheckList::getCalendarData();
        $compliance = \Services\ComplianceDiary::getCalendarData();
        return \Response::json(array_merge($checklist,$compliance));
    }

    public function getDetails($id)
    {
        $task = \Model\ComplianceDiaryTasks::find($id);
        $utz = \Auth::user()->timezone;
    ...
    {
        $this->layout = \View::make($this->layout);
        $item = \Model\ComplianceDiaryItems::find($id);
        if(!$item || !$item -> checkAccess())
            return \Redirect::to('/compliance-diary')->with(['errors'=>\Lang::get('/common/messages.not_exist')]);
        $task = $item -> task;
        ...
        if($form = $task -> form){
            ...
            $options = ['section' => 'compliance_diary_items', 'item_id'=>$id, 'form_id'=>$form->id, 'render'=>true];
    ...
    public function postComplete($id)
    {
        ...
        $input = \Input::all();
        $submitted = new \Model\ComplianceDiarySubmitted();
        ...
        if(!$validator -> fails()) {
            ...
            $save = $submitted -> save();
            if($save) {
                \Services\FilesUploader::updateAfterCreate(['compliance_diary_submitted', $user->id, $unit->id, $submitted->id]);
                $item->delete();
                return \Response::json(['type'=>'success', 'redirect'=>($ref?:'/compliance-diary')]);
    ...
    public function getTasksListDatatable()
    {
        if(!\Request::ajax())
            return $this -> redirectIfNotExist();
        $unitId = \Auth::user() -> unit() -> id;
        $tasks = \Model\ComplianceDiaryTasks::whereUnitId($unitId)->orderBy('title','ASC')->get();
        $options = [];
        if($tasks)
            foreach ($tasks as $task)
            {
                ...
                $options[] = [
                    ...
                    '<div class="text-center">'.\HTML::ownOuterBuilder(
                        \HTML::ownButton($task->id.'/'.$task,'compliance-diary','edit','fa-pencil')
                    ).'</div>',
                ];
            }
        return \Response::json(['aaData' => $options]);
    ...
    public function getDelete($id)
    {
        ...
        if ($delete)
            return \Redirect::to('/compliance-diary')->with('success', \Lang::get('/common/messages.delete_success'));
        else
            return \Redirect::to('/compliance-diary')->with('fail', \Lang::get('/common/messages.delete_fail'));
    }
    ...
    public function getSubmittedDelete($id,$destin)
    {
        ...
        $submitted -> delete();
        return \Redirect::to('/compliance-diary/submitted')->with('success', \Lang::get('/common/messages.delete_success'));
    }
    ...
    public function getDeleteByStaff(){
        ...
        $staffIds = \Model\ComplianceDiaryTasks::whereUnitId(\Auth::user()->unitId())->lists('staff_id');
        if(!$staffIds)
    ...
    public function postDeleteByStaff()
    {
        ...
        if(!$staff || !$staff -> checkAccess())
            return $this->redirectIfNotExist();
        $schedule = \Model\ComplianceDiaryTasks::whereUnitId(\Auth::user()->unitId())->whereStaffId($staff->id);
