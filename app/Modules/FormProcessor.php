...
class FormProcessor extends Modules{
    ...
    public function getFileDelete($id)
    {
        ...
        if($file -> delete())
            return \Response::json(['type'=>'success', 'message'=>\Lang::get('/common/messages.delete_success')]);
        else
        ...
        <div class="uk-button-dropdown" data-uk-dropdown="{mode:\'click\',pos:\'top-left\'}">
            ...
            <div class="uk-dropdown uk-dropdown-bottom">
                <ul class="uk-nav uk-nav-dropdown">
                    ...
                    <li><a class="form-file-download" href="/form-processor/file/download/'.$file->id.'"><i class="material-icons uk-margin-right">file_download</i>Download</a></li>
                    <li class="uk-nav-divider"></li>
                    <li><a data-confirm="ajax-file-delete" href="/form-processor/file/delete/'.$file->id.'"><i class="material-icons uk-margin-right">delete</i>Delete</a></li>
                </ul>
            </div>
        </div>';
    }
    ...
    public function postData()
    {
        ...
        if($form){
            ...
            if(!$validator -> fails())
            {
                ...
                if($assId = \Input::get('cleaning_schedules_items.id')){
                    $assigned = 'cleaning_schedules_items,'.$assId;
                }
                elseif($assId = \Input::get('check_task_items.id')){
                    $assigned = 'check_list_items,'.$assId;
                }
                elseif($assId = \Input::get('compliance_diary_items.id')){
                    $assigned = 'compliance_diary_items,'.$assId;
                }
                else{
                    $assigned = NULL;
                }
                ...
                switch($form -> assigned_id){
                    case '4' :\Services\CleaningSchedule::submitForm($inputs, $answer->id);break;
                    case '2' : case '3' : \Services\CheckList::submitForm($inputs, $answer->id); break;
                }
                $ref = \Session::get('ref-url') ? : null;
                if(!$ref) {
                    if ($assId = \Input::get('cleaning_schedules_items.id')) {
                        $redirect = '/cleaning-schedule';
                    } elseif ($assId = \Input::get('check_task_items.id')) {
                        $redirect = '/check-list';
                    } elseif ($assId = \Input::get('compliance_diary_items.id')) {
                        $redirect = '/compliance-diary';
                    } else {
                        $redirect = '';
                    }
                } else {
    ...
    public function postResolve($id)
    {
        ...
        $errors = $validator -> messages() -> toArray();
        if($errors){
            return \Response::json(['type'=>'error', 'msg'=>\Lang::get('/common/messages.update_fail'), 'form_errors' => $this->ajaxErrors($this->formsErrorsConverter($errors),[])]);
        }
        else {
            $text = '';
            $resolveTypes = ['yes_no'];
            foreach ($resolveTypes as $type) {
                ...
                foreach ($answerTypes as $idAns => $values) {
                    ...
                    if ($checkUpdated > 0){
                        if(!$answer -> getComplaintsAnswers() || ($answer -> getComplaintsAnswers() -> count() == 0)){
                            ...
                            if($update){
                                ...
                                if(count($assigned) == 2){
                                    ...
                                    $submitted = null;
                                    switch ($section){
                                        case 'cleaning_schedules_items' : $submitted = \Model\CleaningSchedulesSubmitted::whereFormAnswerId($answer->id)->whereTaskItemId($itemId)->first(); break;
                        ...
                        return \Response::json(['type'=>'info', 'message'=>'Nothing to update, please change form answers.']);
                    }
                }
            }
        }
