<?php
$start =  Carbon::parse($item->start,'UTC')->timezone($item->task->tz);
$end   =  $item->end ? Carbon::parse($item->end)->timezone($item->task->tz) : $start->copy()->endOfDay();
    ...
    <div class="row m-t">
        <div class="col-sm-12 text-center">
            @if($item->isAbleToComplete())
                <a class=" btn btn-green pull-left col-sm-12 m-b" id="mark-complete" data-dismiss="modal" data-toggle="ajaxModal" href="{{URL::to("/new-cleaning-schedule/complete/$item->id")}}"><i class="fa fa-check"></i> {{Lang::get('common/button.mark_as_complete')}}</a>
            @elseif($submitted)
