...
class CreateNewCleaningSchedules extends Migration {
	...
	public function up()
	{
        Schema::create('new_cleaning_schedules_tasks', function($table)
        {
            $table->engine = 'InnoDB';
            $table->increments  ('id');
        ...
        Schema::create('new_cleaning_schedules_items', function($table)
        {
            $table->engine = 'InnoDB';
            $table->increments('id');
            $table->integer('unit_id')->unsigned();
            ...
            $table->foreign('task_id')->references('id')->on('new_cleaning_schedules_tasks')->onDelete('cascade');
            $table->timestamp('start')->default('0000-00-00 00:00:00');
            $table->timestamp('end')->default('0000-00-00 00:00:00');
            $table->timestamp('expiry')->default('0000-00-00 00:00:00');
            $table->timestamps();
        });
        Schema::create('new_cleaning_schedules_submitted', function($table)
        {
            $table->engine = 'InnoDB';
            $table->increments  ('id');
            $table->integer     ('unit_id')->unsigned();
