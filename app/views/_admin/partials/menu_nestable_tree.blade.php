@foreach ($pageItems as $item)
    ...
    <li class="dd-item  @if ($item['children']) dd-collapsed  @endif " data-id="{{$id}}">
        ...
        <span class="pull-right" style="padding:5px">
            @if($item['page']->ico)
                <i class="{{$item['page']->ico}} text-primary-lt m-r text-muted"></i>
            @endif
        ...
        @if ($item['children'])
            <ol class="dd-list">
                @include('_admin.partials.menu_nestable_tree', ['pageItems'=>$item['children'],'first'=>false, 'refresh'=>$refresh ,'section'=>$section])
            </ol>
        @endif
    </li>
@endforeach