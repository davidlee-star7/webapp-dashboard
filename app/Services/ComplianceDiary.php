...
class ComplianceDiary extends CalendarEvents
{
    function isSubmitted($task, $start)
    {
        return ($start->isPast()) ? (\Model\ComplianceDiarySubmitted::
    ...
    public static function submittedTasks($unitId,$startDate,$endDate)
    {
        $self = new self();
        $dates = [];
        $submitteds = \Model\ComplianceDiarySubmitted::
        ...
        foreach($submitteds as $submitted){
            ...
            if(is_null($submitted->expired_dates)) {
                ...
                $dates['cd-s'.$submitted->id][$startItem->timestamp] = ($self->addDate($submitted, $startItem, $endItem)+['task_status'=>$taskStatus,'item_id'=>$submitted->id]);
            }else{
                ...
                if(count($exploded)){
                    foreach($exploded as $expDate){
                        ...
                        if(($startItem>=$startDate) && ($startItem<=$endDate)) {
                            $dates['cd-s'.$submitted->id][$startItem->timestamp] = ($self->addDate($submitted, $startItem, $endItem)+['task_status'=>4,'item_id'=>$submitted->id]);
                        }
                    }
                }
            }
    ...
    public static function getCalendarData()
    {
        ...
        $tasks    = \Model\ComplianceDiaryTasks::where('unit_id','=',$unitId)-> get();
            ...
            if (in_array($task->repeat, ['day', 'week', 'month', 'year']))
            {
                ...
                foreach ($periodStart as $dpStart)
                {
                    ...
                    if(!$date->isPast() && !$date->isToday() && (($date >= $strQuery) && ($date <= $endQuery)) )
                    {
                        ...
                        $dates['cd-f'.$task->id][$date->timestamp] = ($self->addDate($task, $date, $datex)+['task_status'=>0,'item_id'=>$task->id]);
                    }
                }
            }
            else{
                ...
                {
                    $dates['cd-f'.$task->id][$startRef->timestamp] = ($self->addDate($task,$startRef,$endRef)+['task_status'=>0,'item_id'=>$task->id]);
                }
            }
    ...
    {
        \Model\ComplianceDiaryItems::where('start','>',\Carbon::now())->delete();
        $items = \Model\ComplianceDiaryItems::where('end','<',\Carbon::now())->get();
        foreach($items as $item){
            ...
            $submitted = \Model\ComplianceDiarySubmitted::firstOrCreate($dataSub);
...
/*
    public static function submitForm($inputs,$answer)
    {
        $user = \Auth::user();
        $unitId = $user->unitId();
        $taskItemId = isset($inputs['compliance_diary']['id']) ? $inputs['compliance_diary']['id'] : NULL;
        $answer =  is_int($answer) ? \Model\FormsAnswers::find($answer) : $answer;
        if(!$taskItemId){
            $task = new \Model\ComplianceDiaryTasks();
            $task -> unit_id = $unitId;
            ...
            $task -> save();
            $item =  new \Model\ComplianceDiaryItems();
            $item -> unit_id = $unitId;
        ...
        if ($taskItemId && $answer instanceof \Model\FormsAnswers){
            $answer -> update(['assigned'=>'compliance_diary_items,'.$taskItemId]);
            $unserialize = ($answer && isset($answer->options)) ? unserialize($answer->options) : [];
            if (isset($unserialize['compliant']) && $taskItemId) {
                if ( ($item = \Model\ComplianceDiaryItems::find($taskItemId)) && ($task = $item->task) ) {
