...
class CleaningSchedule extends CalendarEvents
{
    function isSubmitted($task, $start)
    {
        return ($start->isPast()) ? (\Model\CleaningSchedulesSubmitted::
    ...
    public static function submittedTasks($unitId,$startDate,$endDate)
    {
        $self = new self();
        $dates = [];
        $submitteds = \Model\CleaningSchedulesSubmitted::
        ...
        foreach($submitteds as $submitted){
            ...
            if(is_null($submitted->expired_dates)) {
                ...
                $dates['cs-s'.$submitted->id][$startItem->timestamp] = ($self->addDate($submitted, $startItem, $endItem)+['task_status'=>$taskStatus,'item_id'=>$submitted->id]);
            }else{
                ...
                if(count($exploded)){
                    foreach($exploded as $expDate){
                        ...
                        if(($startItem>=$startDate) && ($startItem<=$endDate)) {
                            $dates['cs-s'.$submitted->id][$startItem->timestamp] = ($self->addDate($submitted, $startItem, $endItem)+['task_status'=>4,'item_id'=>$submitted->id]);
                        }
                    }
                }
            }
    ...
    public static function getCalendarData()
    {
        ...
        $tasks    = \Model\CleaningSchedulesTasks::where('unit_id','=',$unitId)-> get();
            ...
            if (in_array($task->repeat, ['day', 'week', 'month', 'year']))
            {
                ...
                foreach ($periodStart as $dpStart)
                {
                    ...
                    if(!$date->isPast() && !$date->isToday() && (($date >= $strQuery) && ($date <= $endQuery)) )
                    {
                        ...
                        $dates['cs-f'.$task->id][$date->timestamp] = ($self->addDate($task, $date, $datex)+['task_status'=>0,'item_id'=>$task->id]);
                    }
                }
            }
            else{
                ...
                {
                    $dates['cs-f'.$task->id][$startRef->timestamp] = ($self->addDate($task,$startRef,$endRef)+['task_status'=>0,'item_id'=>$task->id]);
                }
            }
    ...
    {
        \Model\CleaningSchedulesItems::where('start','>',\Carbon::now())->delete();
        $items = \Model\CleaningSchedulesItems::where('end','<',\Carbon::now())->get();
        foreach($items as $item){
            ...
            $submitted = \Model\CleaningSchedulesSubmitted::firstOrCreate($dataSub);
    ...
    public static function submitForm($inputs,$answer)
    {
        $user = \Auth::user();
        $unitId = $user->unitId();
        $taskItemId = isset($inputs['cleaning_schedules_items']['id']) ? $inputs['cleaning_schedules_items']['id'] : NULL;
        $answer =  is_int($answer) ? \Model\FormsAnswers::find($answer) : $answer;
        if(!$taskItemId){
            $task = new \Model\CleaningSchedulesTasks();
            $task -> unit_id = $unitId;
            ...
            $task -> save();
            $item =  new \Model\CleaningSchedulesItems();
            $item -> unit_id = $unitId;
        ...
        if ($taskItemId && $answer instanceof \Model\FormsAnswers){
            $answer -> update(['assigned'=>'cleaning_schedules_items,'.$taskItemId]);
            $unserialize = ($answer && isset($answer->options)) ? unserialize($answer->options) : [];
            if (isset($unserialize['compliant']) && $taskItemId) {
                if ( ($item = \Model\CleaningSchedulesItems::find($taskItemId)) && ($task = $item->task) ) {
