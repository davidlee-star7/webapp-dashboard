...
@section('content')
    <div class="m-b-md">
        <h3 class="m-b-none"><i class="i i-arrow-down3 m-r"></i>{{$sectionName}}
           ...
           <a class="btn btn-green" href="{{URL::to("/trainings/list")}}"><i class="fa fa-search"></i> {{Lang::get('common/general.datatable')}} </a>
        </span>
        </h3>
    </div>
                                ...
                                @foreach($staff->skip($i * $limit)->take($limit)->get() as $row)
                                    <div class="col-sm-3 pull-left m-b">
                                        ...
                                        <span class="avatar">
                                            ...
                                            <span style="display: inline-block;" class="badge badge-sm up bg-danger count h3">{{$row->trainingsRecords()->count()}}</span>
                                        </span>
