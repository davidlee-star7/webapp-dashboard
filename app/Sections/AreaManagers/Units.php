...
class Units extends AreaManagersSection {
        ...
        if (count($units)){
            foreach ($units as $unit)
            {
                ...
                $options[] = [
                    ...
                    \HTML::ownOuterBuilder(
                        \HTML::ownIcoStatus($unit->active)
                    ),
                    \HTML::ownOuterBuilder(
                        \HTML::ownButton($unit -> id, 'units', 'edit','fa-pencil','btn-primary')
                    ),
                    \HTML::ownOuterBuilder(
                        \HTML::ownButton($unit -> id, 'units', 'view','fa-search','btn-primary')
                    ),
                ];
            }
        }
    ...
    public function getView($id)
    {
        ...
        if(!$unit || !$unit->checkAccess())
            return $this -> redirectIfNotExist();
        $breadcrumbs = $this -> breadcrumbs -> addLast( $this -> setAction('View',false) );
        return \View::make($this->regView('view'), compact('breadcrumbs','unit'));
    }
