...
class Users extends AreaManagersSection {
    ...
    public function getCreate()
    {
        ...
        $units = $this -> auth_user -> units -> lists('name','id');
    ...
    {
        $role = ($rolem = \Input::get('role')) ? (in_array($rolem,$this->roles)?$rolem:'local-manager') : 'local-manager';
        if(!\Request::ajax())
            return $this->redirectIfNotExist();
        $users = \User::
        whereHas(
            'units', function($query) {
             $query -> whereIn('unit_id', $this->auth_user->units->lists('id'));
        }) -> whereHas(
            ...
            foreach ($users as $user)
            {
                $options[] = [
                    ...
                    '<div class="text-center">'.(\HTML::ownOuterBuilder(
                        \HTML::ownIcoStatus($user->active)
                    )).'</div>',
                    '<div class="text-center">'.(\HTML::ownOuterBuilder(
                        \HTML::ownIcoStatus($user->confirmed)
                    )).'</div>',
                    '<div class="text-center">'.(\HTML::ownOuterBuilder(
