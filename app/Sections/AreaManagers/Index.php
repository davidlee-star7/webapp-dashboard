...
class Index extends AreaManagersSection {
    ...
    public function getIndex()
    {
        ...
        return \View::make($this->regView('index'), 
            compact('breadcrumbs','foodIncidents','statsData','dormantUsers',
                'invalidTemps','navinotes', 'unitScores' ,'dateFrom'));
    }
    ...
    {
        $rolesArr = ['visitor','local-manager'];
        $users = \User::
               whereHas(
                'units', function($query) {
                $query-> whereIn('unit_id', $this -> getUnitsId());
                ...
                foreach($scores as $score){
                    if($score->target_type == $item){
                        if($score->target_type == 'temperatures_for_pods'){
                            ...
                            $datas[$target->area->id.$item]=['label' => $target->area->name, 'data'=> $total[$item][$target->area_id]];
                        }
                        else{
