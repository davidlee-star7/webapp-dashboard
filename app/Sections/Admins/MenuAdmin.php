...
class MenuAdmin extends AdminsSection {

    public $targetType = 'admin';
    protected $structure;
    protected $_root;
    protected $types;
    ...
    public function postCreate()
    {
        ...
        if(!$validator -> fails())
        {
            ...
            return \Redirect::to('/menu-admin')->with($type, \Lang::get('/common/messages.create_'.$type));
        }
        else
        {
    ...
    public function postEdit($id)
    {
        ...
        if(!$validator -> fails())
        {
            ...
            return \Redirect::to('/menu-admin')->with($type, \Lang::get('/common/messages.update_'.$type));
        }
        else
        {
            return \Redirect::back()->withInput()->withErrors($validator);
    ...
    public function getDelete($id)
    {
        ...
        if($structure->hasChildren()){
            return \Redirect::to('/menu-admin')->withErrors(\Lang::get('/common/messages.remove_childrens'));
        }
        else{
            $delete = $structure -> delete();
            $type   = $delete ? 'success' : 'fail';
            return \Redirect::to('/menu-admin')->with($type, \Lang::get('/common/messages.delete_'.$type));
        }
    }
