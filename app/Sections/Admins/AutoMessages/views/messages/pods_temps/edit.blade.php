                    ...
                    <div class="col-sm-12">
                        <section class="panel panel-default">
                            <div class="panel-body">
                                <div class="row">
                                    <form role="form" action="{{URL::to('/auto-messages/msg/'.$message->id.'/edit')}}" method="post">
                                        <div class="form-group col-sm-12">
                                            <div class="row">
                                                <div class="col-sm-12">
                                                    <label class="font-bold">{{\Lang::get('/common/general.title')}}</label>
                                                    <input name="title" type="text" value="{{Input::old('title', $message->title)}}" placeholder="{{\Lang::get('/common/general.title')}}" class="form-control">
                                                </div>
                                                <div class="col-sm-12">
                                                    <label class="font-bold">{{\Lang::get('/common/general.message')}}</label>
                                                    <textarea name="message" class="form-control" wyswig="basic">{{Input::old('title', $message->message)}}</textarea>
                                                    <label class="font-bold">{area_name},{unit_name},{temperature},{valid_range},{warning_range}</label>
                                                </div>
                                            </div>
                                        </div>
