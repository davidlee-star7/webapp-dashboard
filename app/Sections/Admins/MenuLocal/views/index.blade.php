...
<div class="m-b-md">
    <h3 class="m-b-none"><i class="i i-arrow-down3 m-r"></i>{{$sectionName}}
        <span class="pull-right">
               <a class="btn btn-green" href="{{URL::to('/menu-local/create')}}"><i class="fa fa-plus fa-fw"></i>Create</a>
    ...
    <div class="row m-b">
        <div class="">
            <div class="dd nestable" id="nestable" data-max_depth="{{$maxLevels}}" >
                <ol class="dd-list" data-url="{{URL::to('/menu-local/edit/update')}}">
                    @include('_admin.partials.menu_nestable_tree', ['pageItems'=>$tree, 'first'=>true, 'refresh'=>$refresh=0, 'section'=>'menu-local'])
                </ol>
            </div>
        </div>
    </div>
