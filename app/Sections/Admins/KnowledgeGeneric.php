...
class KnowledgeGeneric extends AdminsSection {
    ...
    public function postCreate()
    {
        ...
        if(!$validator -> fails())
        {
            ...
            return \Redirect::to('/knowledge-generic')->with($type, \Lang::get('/common/messages.create_'.$type));
        }
        else
        {
    ...
    public function postEdit($knowledge)
    {
        ...
        if(!$validator -> fails())
        {
            ...
            return \Redirect::to('/knowledge-generic')->with($type, \Lang::get('/common/messages.update_'.$type));
        }
        else
        {
            return \Redirect::back()->withInput()->withErrors($validator);
    ...
    public function getDelete($knowledge)
    {
        ...
        if($knowledge->hasChildren()){
            return \Redirect::to('/knowledge-generic')->withErrors(\Lang::get('/common/messages.remove_childrens'));
        }
        else{
            $delete = $knowledge -> delete();
            $type   = $delete ? 'success' : 'fail';
            return \Redirect::to('/knowledge-generic')->with($type, \Lang::get('/common/messages.delete_'.$type));
        }
    }
