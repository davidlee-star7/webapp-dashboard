...
class HaccpCompany extends HqManagersSection {

    protected $haccp;
    protected $_root;
    public $targetType = 'specific';
    ...
    public function __construct(\Model\Haccp $haccp)
    {
        ...
        $this -> breadcrumbs -> addCrumb('haccp-company', 'Haccp Company');
    }
    ...
    public function postCreate()
    {
        ...
        if(!$validator -> fails())
        {
            ...
            return \Redirect::to('/haccp-company')->with($type, \Lang::get('/common/messages.create_'.$type));
        }
        else
        {
    ...
    public function postEdit($id)
    {
        ...
        if(!$validator -> fails())
        {
            ...
            return \Redirect::to('/haccp-company')->with($type, \Lang::get('/common/messages.update_'.$type));
        }
        else
        {
            return \Redirect::back()->withInput()->withErrors($validator);
    ...
    public function getDelete($id)
    {
        ...
        if($haccp->hasChildren()){
            return \Redirect::to('/haccp-company')->withErrors(\Lang::get('/common/messages.remove_childrens'));
        }
        else{
            $delete = $haccp -> delete();
            $type   = $delete ? 'success' : 'fail';
            return \Redirect::to('/haccp-company')->with($type, \Lang::get('/common/messages.delete_'.$type));
        }
    }
    ...
    public function getDatatable($refresh=null)
    {
        ...
        return \View::make('Sections\HqManagers\HaccpCompany::nestable_tree', compact('pageItems','first','refresh'));
    }
